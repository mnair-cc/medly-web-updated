...
const ManageAccountModal = ({ isOpen, onClose }: ManageAccountModalProps) => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const updateData: { userName?: string; year?: number } = {};

      if (editData.userName.trim() !== user.userName) {
        updateData.userName = editData.userName.trim();
      }

      if (editData.year !== user.year) {
        updateData.year = editData.year;
      }

      // At this point we know there are changes, so updateData should not be empty
  ...
  const hasChanges =
    user &&
    (editData.userName.trim() !== user.userName || editData.year !== user.year);
