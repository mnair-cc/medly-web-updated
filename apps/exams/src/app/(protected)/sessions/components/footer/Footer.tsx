...
import SketchToolbarTutorialTooltip from "./SketchToolbarTutorialTooltip";
...
const Footer = ({
  ...
  const encouragementText = useMemo(() => {
    ...
    const difficultyEncouragements =
      encouragements[difficulty] || encouragements[QuestionDifficulty.EASY];
  ...
  const shouldShowFooterTools =
    !currentQuestionWithMarkingResult?.legacyId?.includes("EngLang") &&
    !currentQuestionWithMarkingResult?.legacyId?.includes("EngLit");

  // Calculate isMarked and other derived values first
  // Use optional chaining to handle null questionGroup
  ...
  // Don't render footer for textbook or learn pages
  if (
    pages[currentPageIndex]?.type === QuestionSessionPageType.Textbook ||
    pages[currentPageIndex]?.type === QuestionSessionPageType.Learn
  ) {
    return null;
  }

  // For mock sessions on cover page, allow footer to render for Start button
  const isMockCoverPage =
    sessionType === SessionType.MockSession &&
    pages[currentPageIndex]?.type === QuestionSessionPageType.Cover;

  // Guard: questionGroup should not be null after textbook check
  // Exception: allow mock session cover page to render footer for Start button
  if (!questionGroup && !isMockCoverPage) {
    return null;
  }
      ...
      >
        <div className="flex justify-between items-center">
          <div className="flex-1">
            {breakdownIsMarked && (
              <div
                className={`px-4 font-rounded-heavy flex items-center gap-3 ${
  ...
  return (
    <div
      className={`flex flex-col justify-center px-4 py-4 z-[1005] min-h-24 absolute bottom-0 right-0 w-full pointer-events-none ${
        shouldAnimate ? "animate-slide-up ease-out duration-300" : ""
      }
        ${
          ((isCalculatorOpen &&
      ...
      // }}
    >
      {shouldShowFooterTools && !isMarked && isWideScreen && questionGroup && (
        <>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-row">
            <SketchToolbar
              ...
              type={
                isTouchScreen &&
                questionGroup.questions[0].legacyId?.includes("Math")
                  ? "toolbar-math"
                  : "toolbar"
              }
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center">
        <div className={`flex justify-between items-center gap-2`}>
          ...
          {isMarked &&
            ...
            (isReadOnly ||
              ...
              <div
                className={`px-4 font-rounded-heavy flex items-center gap-3 mb-4 md:mb-0 ${
                  isAllCorrect
                    ? "text-[#7CC500]"
                    : isAllIncorrect
                      ? "text-[#FF4B4C]"
          ...
          {isMarked && (
            <div className="flex items-center gap-2 pointer-events-auto">
              <button
                className={`flex items-center ml-4 gap-1 font-rounded-bold mb-4 md:mb-0 ${
                  isAllCorrect
                    ? "text-[#7CC500]"
                    : isAllIncorrect
                      ? "text-[#FF4B4C]"
        ...
        {/* {sessionType === SessionType.PaperSession &&
          pages[currentPageIndex].type === QuestionSessionPageType.Question && (
            <div className="flex-1 flex justify-center">
              <div className="relative flex flex-row items-center gap-2 mb-4 md:mb-0">
                <button
                  ref={toggleQuestionInfoButtonRef}
                  className="flex flex-row items-center gap-2 bg-white rounded-full px-4 py-2"
                  onClick={() => setShowQuestionInfo(!showQuestionInfo)}
                >
                  <p
                    className={`font-rounded-semibold ${showQuestionInfo ? "text-black" : "text-black/50"
                      }`}
                  >
                    {currentQuestionNumber
                      ? `Question ${currentQuestionNumber} of ${questionPages.length}`
                      : "Review"}
                  </p>
                  {showQuestionInfo ? (
                    <ChevronDownIcon />
                ...
                {showQuestionInfo && (
                  <QuestionReviewCard
                    ...
                    toggleQuestionInfoButtonRef={toggleQuestionInfoButtonRef}
                    isReadOnly={isReadOnly}
                  />
                )}
              </div>
              {canUseSketchToolbar && isToolbarVisible && (
                <SketchToolbar
                  mode={inputMode}
        ...
        <div className="w-full md:w-auto md:flex-1 flex flex-col-reverse md:flex-row gap-2 justify-end">
          {sessionType === SessionType.MockSession && canMoveToPreviousPage && (
            ...
            <div className="z-[1001] flex justify-center items-center">
              {(sessionType === SessionType.PracticeSession ||
                ...
                <SolveTogetherButton
                  ...
                  disabled={isWideScreen && isSolveTogether && !showWhyTooltip}
                  isLoading={false}
                />
              )}
          ...
          {canFinishSession && sessionType !== SessionType.PaperSession && (
            <FinishSessionButton
              onPress={() => {
