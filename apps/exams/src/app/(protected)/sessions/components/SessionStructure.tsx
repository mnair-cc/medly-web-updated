...
const SessionStructure = ({
  ...
  const { isTextbookUnlocked, isSubjectUnlocked } = useMedlyMondays();
        ...
        <div className="flex flex-row flex-1 w-full h-full">
          ...
          >
            ...
            >
              ...
              {/* Lock overlay inside content area - doesn't cover header nav or sidebar */}
              {/* Don't show lock on textbook pages during Medly Mondays when textbook is unlocked */}
              {/* Don't show lock on Learn pages - LearnPage.tsx handles its own locking */}
              {/* Don't show lock for LessonSession when subject is unlocked (but papers stay locked) */}
              {(sessionType === SessionType.LessonSession ||
                sessionType === SessionType.PaperSession) &&
                !hasActivePlan &&
                ...
                sessionData.pages[currentPageIndex]?.type !==
                  QuestionSessionPageType.Learn &&
                // Skip lock for LessonSession when subject is unlocked via Medly Mondays
                !(
                  sessionType === SessionType.LessonSession &&
                  isSubjectUnlocked(subjectLegacyId)
                ) &&
                !(
                  sessionData.pages[currentPageIndex]?.type ===
                        ...
                        (questionGroup.questions as QuestionWithMarkingResult[])
                          ...
                          <AiGroupCard
                            ...
                            showPremiumPrompt={
                              ...
                              !hasActivePlan &&
                              featureUsage.isFreeUseFinished &&
                              // Don't show premium prompt for LessonSession when subject is unlocked
                              !(
                                sessionType === SessionType.LessonSession &&
                                isSubjectUnlocked(subjectLegacyId)
                              )
