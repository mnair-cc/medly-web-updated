...
/**
 * Get the current mock date scenario from PostHog feature flag.
...
/**
 * Calculate the time offset for a mock scenario.
...
/**
 * Check if a mock date scenario is currently active.
...
/**
 * Get the subject's mock exam start date, adjusted for any active test scenario.
 *
...
/**
 * Get a mock milestone date in UTC, adjusted for any active test scenario.
 *
 * When a PostHog mock_date_scenario is active, the returned date is offset
 * so that moment().isAfter/isBefore comparisons behave as if the current
...
// =============================================================================
// These functions work in both client and server components. They handle
// PostHog feature flag lookups internally - callers don't need to know about auth.
...
export const isAnySubjectDayActiveAsync = async (): Promise<boolean> => {
  // On client, use sync version
  if (typeof window !== "undefined") {
    return isAnySubjectDayActive();
  }
  // On server, use async PostHog lookup
  const scenario = await getServerScenario();
