...
Open platform data access: `/open` pages and APIs use Drizzle with Postgres directly. Firestore is deprecated for the open platform.
...
Drizzle manages only `open_platform_*` tables. Other tables in the database are handled by the Python backend (Alembic).
...
1. Edit `src/db/migrations/schema.ts` - add table (must be `open_platform_*`)
2. Run `npm run db:generate` - creates migration SQL
3. Run `npm run db:migrate` - applies to local DB
4. Update `src/db/types.ts` with the corresponding type exports
5. Commit schema + migration

### Notes

- Only `open_platform_*` tables are managed by Drizzle
- Other tables in the DB (curriculum, learn-flow, etc.) are ignored
- The baseline migration uses `IF NOT EXISTS` for safe deployment on existing databases
