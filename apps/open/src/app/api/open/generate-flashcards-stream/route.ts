...
import { generateFlashcardsStream } from "@/app/(protected)/open/_ai/generateFlashcards";
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          const generator = generateFlashcardsStream(sourceContent, {
            count: count ?? 10,
            instructions,
          });

          for await (const card of generator) {
            const data = JSON.stringify({ type: "card", card });
            controller.enqueue(encoder.encode(`data: ${data}\n\n`));
          }

          controller.enqueue(encoder.encode(`data: {"type":"done"}\n\n`));
        } catch (error) {
          console.error("[generate-flashcards-stream] Error:", error);
    ...
    console.error("[generate-flashcards-stream] Request error:", error);
    return new Response(
