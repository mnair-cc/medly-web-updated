...
const items: SelectorItem[] = [
  {
    ...
    icon: TextIcon,
    command: (editor) => {
      if (editor) editor.chain().focus().toggleNode("paragraph", "paragraph").run();
    },
  ...
  },
  {
    ...
    icon: TextQuote,
    command: (editor) => {
      if (editor) editor.chain().focus().toggleNode("paragraph", "paragraph").toggleBlockquote().run();
    },
...
export const NodeSelector = ({ open, onOpenChange }: NodeSelectorProps) => {
  ...
  return (
    <Popover modal={true} open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger
        asChild
        className='gap-2 rounded-none border-none hover:bg-gray-100 focus:ring-0'>
