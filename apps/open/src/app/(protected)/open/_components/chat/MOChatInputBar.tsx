 ...
 * 
 * This file is ~90% duplicated with MobileMOChatInputBar.tsx (~700 lines each).
 * 
 * RECOMMENDED PATTERN (Hybrid Approach):
 * 1. Extract ALL business logic to: useInputBarLogic.ts
 *    - State: userInput, skills menu, validation
 *    - Handlers: handleSend, handleKeyDown, handleFileSelect
 *    - Returns: { userInput, setUserInput, handleSend, ... }
 * 
 * 2. Keep SINGLE component file with layout variants:
 *    - MOChatInputBar.tsx:
 *      export function InputBar({ variant = "default", ...props }) {
 *        const logic = useInputBarLogic();
 *        const Layout = variant === "mobile" ? MobileLayout : DesktopLayout;
 *        return <Layout {...logic} {...props} />;
 *      }
 *    - Internal components (same file): DesktopLayout, MobileLayout (UI only)
 * 
 * WHY: Single maintainer can see both layouts in one file, logic can't diverge.
 * 
 * RISK: Currently, bug fixes/features must be manually applied to BOTH files.
 */

import { useRef, useLayoutEffect, useState, useEffect, useMemo } from "react";
import PrimaryButtonClicky from "@/app/_components/PrimaryButtonClicky";
import Spinner from "@/app/_components/Spinner";
import type { SelectedScreenshot, DraggedContextItem } from "./MOChatLayoutClient";
...
function InputBar({
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full pointer-events-auto relative">
      ...
      <div className={`flex flex-col w-full rounded-[18px] pointer-events-auto ${highlightInput ? "border-2 border-[#1CA4FF]" : ""} ${backgroundColor ? `bg-[${backgroundColor}]` : ""} 
      ${style === "flat" ? "bg-[#F2F2F7]" : "shadow-[0_0_10px_rgba(0,0,0,0.08)]"}`}>
        {/* Selected text row */}
        ...
        {/* Input row */}
        <div className="flex items-center justify-center w-full min-h-[40px] p-2 md:p-1 px-2 relative">
          ...
          <textarea
            ...
            value={userInput}
            className="w-full bg-transparent outline-none pl-2.5 py-2.5 text-base md:text-sm resize-none overflow-hidden font-rounded-semibold min-h-[40px] placeholder:text-black/40"
            rows={1}
            onChange={(e) => {
          ...
          />
          <div className="flex items-end self-end md:mb-[5px] md:mr-2">
            <button
              className={`w-[28px] h-[28px] rounded-full flex flex-col justify-center items-center ${canReply ? "bg-[#00AEFF]" : "bg-[#B3B3B3]"}`}
              disabled={!canReply}
              onClick={() => {
                if (canReply) {
                  // Check if current skill has autoSend enabled
