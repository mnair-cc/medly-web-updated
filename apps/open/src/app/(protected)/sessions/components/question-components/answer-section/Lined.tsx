...
const Open = ({
  ...
  //         {type === 'lined' ? (
  //           <div
  //             className="outline-none resize-none p-4 md:mb-2 overflow-y-hidden text-[18px] md:text-[18px] font-['Shantell_Sans'] font-[500]"
  //             style={{
  ...
  const formatTextWithHighlighting = (text: string) => {
    ...
    if (currentQuestionWithMarkingResult.annotations) {
      ...
      if (typeof currentQuestionWithMarkingResult.annotations === "string") {
        // Parse string format
        const annotationsString = currentQuestionWithMarkingResult.annotations;
      ...
      // Handle annotations as direct object
        ...
        typeof currentQuestionWithMarkingResult.annotations === "object"
      ) {
      ...
      if (annotations?.weak && annotations.weak.length > 0) {
        annotations.weak.forEach((weakText) => {
          if (processedText.includes(weakText)) {
            ...
            const regex = new RegExp(`(\\*\\*)?${weakText}(\\*\\*)?`, "g");
    ...
    return parts.map((part, i) => {
      if (part.type === "bold") {
        return (
          <span key={i} className="bg-[#E6FFBC] py-0.5 px-1 -m-1 rounded-[4px]">
            {part.content}
          </span>
        );
      } else if (part.type === "code") {
        return (
          <span key={i} className="bg-[#FFC0C0] py-0.5 px-1 -m-1 rounded-[4px]">
            {part.content}
          </span>
        );
      }
        ...
        <div className="flex flex-col gap-2">
          {type === "lined" ? (
            <textarea
              ...
              style={{
                minHeight: `${(currentQuestionWithMarkingResult.maxMark || 1) *
                  (currentQuestionWithMarkingResult.legacyId?.includes("Maths")
                    ? 4
                    : 2) *
                  30
                  }px`,
              ...
              id="userAnswer"
              name="userAnswer"
              placeholder="Write your answer in a Word document and paste your answer here"
            ...
            <textarea
              className="outline-none resize-none bg-[#F7F7FB] rounded-[16px] border border-[#EFEFF6] p-4 md:mb-2 overflow-y-hidden text-[18px] md:text-[18px] font-['Shantell_Sans'] font-[500]"
              style={{ height: "auto" }}
              id="userAnswer"
              name="userAnswer"
        ...
        {type === "lined" ? (
          ...
          >
            {typeof userAnswer === "string" &&
              formatTextWithHighlighting(userAnswer)}
          </div>
        ) : (
          <div className="outline-none resize-none pb-8 bg-[#F7F7FB] rounded-[16px] border border-[#EFEFF6] p-4 mb-2 text-[18px] md:text-[18px] font-['Shantell_Sans'] font-[500]">
            {typeof currentQuestionWithMarkingResult.annotatedAnswer ===
              "string" &&
              currentQuestionWithMarkingResult.annotatedAnswer.length >=
              0.75 *
              (typeof currentQuestionWithMarkingResult.userAnswer === "string"
                ? currentQuestionWithMarkingResult.userAnswer.length
                : 0) ? (
                ...
                .map((part: string, index: number) => {
                  if (part.startsWith("**") && part.endsWith("**")) {
                    return (
                      <span
                        key={index}
                        className="bg-[#FFC0C0] py-0.5 px-1 -m-1"
                      >
                        {part.slice(2, -2)}
                      </span>
                    );
                  ...
                  if (part.startsWith("`") && part.endsWith("`")) {
                    return (
                      <span
                        key={index}
                        className="bg-[#E6FFBC] py-0.5 px-1 -m-1"
                      >
                        {part.slice(1, -1)}
                      </span>
                    );
              ...
              <span>
                {typeof currentQuestionWithMarkingResult.userAnswer ===
