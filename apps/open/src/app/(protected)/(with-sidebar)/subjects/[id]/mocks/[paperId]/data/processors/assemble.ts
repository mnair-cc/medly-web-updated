...
import { MockData } from "../fetchers/curriculum";
import { calculateQuestionGroupProgress } from "@/app/(protected)/sessions/utils";

export function assembleSessionData(
  paperId: string,
  questionsWithMarkingResults: QuestionWithMarkingResult[],
  mockDetails: MockData,
  ...
  // Calculate total marks before creating cover content
  const totalMarksPossible = questionsWithMarkingResults.reduce(
    (sum, q) => sum + q.maxMark,
    0
  );

  // Determine tier from gcseHigher
  const tier: "Higher" | "Foundation" | null =
    mockDetails.gcseHigher === true
      ? "Higher"
      : mockDetails.gcseHigher === false
        ? "Foundation"
        : null;

  // Create cover page for mock sessions
  const coverContent: CoverContent = {
    examBoard: mockDetails.examBoard,
    subject: mockDetails.subject,
    course: mockDetails.course,
    series: mockDetails.series,
    paper: mockDetails.number,
    date: mockDetails.date,
    time: `${mockDetails.durationInMinutes} minutes`,
    tier,
    totalMarks: totalMarksPossible,
  };

  const coverPage = {
    type: QuestionSessionPageType.Cover,
    content: coverContent,
    progress: 0,
  };

  const questionPages = questionGroups.map((questionGroup) => ({
    type: QuestionSessionPageType.Question,
    content: questionGroup,
    progress: calculateQuestionGroupProgress(questionGroup),
  }));

  const pages = [coverPage, ...questionPages];
  ...
  return {
    id: existingSessionData?.id || `mock-${paperId}-${Date.now()}`,
    sessionType: SessionType.MockSession,
    sessionTitle: mockDetails.title || `Mock ${mockDetails.number}`,
    sessionSubtitle: `${mockDetails.examBoard} • ${mockDetails.course}`,
    ...
    isTimed: true,
    durationInMinutes:
      existingSessionData?.durationInMinutes || mockDetails.durationInMinutes,
    ...
    initialInsights: existingSessionData?.initialInsights,
    resultsDayInsights: existingSessionData?.resultsDayInsights,
  };
}
