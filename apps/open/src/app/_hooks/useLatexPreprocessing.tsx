...
export function preprocessLaTeX(content: string): string {
  ...
  processedContent = processedContent.replace(
    /!\[(.*?)\]\((.*?)\)/g,
    (match, alt, src) => {
      return `![](${src})\n<div style="font-style: italic; color: rgba(0,0,0,0.5); text-align: center; width: 70%; margin: 8px auto 16px; font-size: 14px; font-family: 'Helvetica Neue', sans-serif;">${alt}</div>`;
    }
  );
...
// Function to remove alt text divs from images
export function removeAltText(content: string): string {
  return content.replace(
    /!\[\]\((.*?)\)\n<div style="font-style: italic; color: rgba\(0,0,0,0.5\); text-align: center; width: 70%; margin: 8px auto 16px; font-size: 14px; font-family: 'Helvetica Neue', sans-serif;">(.*?)<\/div>/g,
    (_, src) => {
      return `![](${src})`;
    }
  );
