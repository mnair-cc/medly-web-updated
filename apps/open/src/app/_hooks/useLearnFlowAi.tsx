...
const extractMarkschemeFromBlocks = (
  ...
  for (let i = currentBlockIndex; i >= 0; i--) {
    ...
    if (
      ...
      const correctIndex = block.content.correct_option_index;
  ...
  const handleSendMessage = useCallback(
    async (message: string) => {
      try {
          ...
          visibleBlocks.forEach((block, index) => {
            ...
            if (blockKey && learnFlowProgress.blocks[blockKey]) {
              ...
              if (
                ...
                const correctOptionIndex = block.content.correct_option_index;
                const correctAnswer =
        ...
        // Get current block's canvas (prefer from progress if available)
        const currentBlockKey = blockKeyMap?.get(currentBlockIndex);
        const currentBlockCanvas =
          currentBlockKey && learnFlowProgress?.blocks[currentBlockKey]?.canvas
            ? learnFlowProgress.blocks[currentBlockKey].canvas
            : currentCanvas[0] || { paths: [], textboxes: [], maths: [] };
